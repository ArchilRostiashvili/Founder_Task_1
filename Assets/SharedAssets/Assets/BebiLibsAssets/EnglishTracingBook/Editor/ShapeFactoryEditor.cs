...
public class ShapeFactoryEditor : EditorWindow
{
    ...
    private void ShowPathsSection()
    {
        ...
        for (int i = 0; i < paths.Count; i++)
        {
            ...
            paths[i].sprite = EditorGUILayout.ObjectField("Sprite", paths[i].sprite, typeof(Sprite), true) as Sprite;
            EditorGUILayout.Separator();
            ...
            paths[i].number1 = EditorGUILayout.ObjectField("First Number", paths[i].number1, typeof(Sprite), true) as Sprite;
            EditorGUI.EndDisabledGroup();
            ...
            if (!paths[i].enableNumber2)
                EditorGUI.BeginDisabledGroup(true);
            paths[i].number2 = EditorGUILayout.ObjectField("Second Number", paths[i].number2, typeof(Sprite), true) as Sprite;
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.EndHorizontal();
