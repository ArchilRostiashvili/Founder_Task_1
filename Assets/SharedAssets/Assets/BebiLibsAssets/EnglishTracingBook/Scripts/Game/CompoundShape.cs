...
namespace IndieStudio.EnglishTracingBook.Game
{
    ...
    public class CompoundShape : MonoBehaviour
    {
        ...
        public int GetShapeIndexByInstanceID(int ID)
        {
            for (int i = 0; i < this.shapes.Count; i++)
            {
                if (ID == this.shapes[i].GetInstanceID())
                {
                    return i;
                }
            }
        ...
        {
            this.transform.localScale = Vector3.one * this.scaleFactor * CommonUtil.ShapeGameAspectRatio();
        }

        public void ApplyOffset()
        {
            this.transform.localPosition = new Vector3(this.sentenceOffset.x, this.sentenceOffset.y, 0);
        }
        ...
        public void Generate()
        {
            ...
            this.gameObject.name = this.text.Replace('\n', ' ') + "-sentence";
            ...
            tempCharacterOffset.x = this.characterOffset.x * this.scaleFactor * CommonUtil.ShapeGameAspectRatio();
            tempCharacterOffset.y = this.characterOffset.y * this.scaleFactor * CommonUtil.ShapeGameAspectRatio();
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                //new line
                yPos = this.transform.position.y + Mathf.FloorToInt(lines.Length / 2.0f) * tempCharacterOffset.y - (i * tempCharacterOffset.y) - extraOffset;
                ...
                for (int j = 0; j < lines[i].Length; j++)
                {
                    ...
                    if (shapeExists)
                    {
                        ...
                        xPos = this.transform.position.x - Mathf.FloorToInt(lines[i].Length / 2.0f) * tempCharacterOffset.x + (j * tempCharacterOffset.x) + extraOffset;

                        //create new shape
                        this.CreateNewShape(shapePrefab, new Vector3(xPos, yPos, 0));
                    }
                }
            }
        }
