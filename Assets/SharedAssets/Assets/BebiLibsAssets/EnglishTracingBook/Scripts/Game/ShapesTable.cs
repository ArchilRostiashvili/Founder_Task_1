...
#pragma warning disable 0219 // variable assigned but not used.
    ...
    public class ShapesTable : MonoBehaviour
    {
        ...
        /// </summary>
        private readonly Transform lastShape;
        ...
        void Start()
        {
            ...
            if (this.albumTitle != null)
                this.albumTitle.sprite = ShapesManager.GetCurrentShapesManager().albumTitle;

            //Create new shapes
            this.StartCoroutine("CreateShapes");
        ...
        void Update()
        {
            if (this.lastShape != null)
            {
                ...
                if (!Mathf.Approximately(this.lastShape.position.magnitude, this.shapeBright.position.magnitude))
                {
                    this.shapeBright.position = this.lastShape.position;
                }
            }
        }
        ...
        private IEnumerator CreateShapes()
        {
            ...
            this.pointersParent.gameObject.SetActive(false);
            this.groupsParent.gameObject.SetActive(false);
            ...
            for (int i = 0; i < ShapesManager.GetCurrentShapesManager().shapes.Count; i++)
            {
                ...
                if (i % this.shapesPerGroup == 0)
                {
                    groupIndex = (i / this.shapesPerGroup);
                    shapesGroup = Group.CreateGroup(this.shapesGroupPrefab, this.groupsParent, groupIndex, this.columnsPerGroup);
                    if (!this.EnableGroupGridLayout)
                    {
                        shapesGroup.GetComponent<GridLayoutGroup>().enabled = false;
                    }
                    if (this.createGroupsPointers)
                    {
                        Pointer.CreatePointer(groupIndex, shapesGroup, this.pointerPrefab, this.pointersParent);
                    }
                }

                //Create new table Shape
                ID = (i);//the id of the shape
                GameObject tableShapeGameObject = Instantiate(this.shapePrefab, Vector3.zero, Quaternion.identity) as GameObject;
                ...
                this.SettingUpTableShape(ShapesManager.GetCurrentShapesManager().shapes[ID], tableShapeComponent, ID, groupIndex);
            }

            this.collectedStarsText.text = ShapesManager.GetCurrentShapesManager().GetCollectedStars() + "/" + (3 * ShapesManager.GetCurrentShapesManager().shapes.Count);
            ...
            this.pointersParent.gameObject.SetActive(true);
            this.groupsParent.gameObject.SetActive(true);
        ...
        private void SettingUpTableShape(ShapesManager.Shape shape, TableShape tableShape, int ID, int groupIndex)
        {
            ...
            if (!ShapesManager.GetCurrentShapesManager().testMode)
                if (!shape.isLocked)
                {
                    if (PlayerPrefs.HasKey(DataManager.GetLockedStrKey(ID + 1, ShapesManager.GetCurrentShapesManager())))
                    {
                        ...
                        {
                            this.SetSelectedGroup(groupIndex);
                        }
                    }
            ...
            this.tempTransform = tableShape.transform.Find("Stars");

            {//Three Stars
                Debug.Log("Starr Burst");
                this.tempTransform.Find("FirstStar").GetComponent<Image>().sprite = this.starOn;
                this.tempTransform.Find("SecondStar").GetComponent<Image>().sprite = this.starOn;
                this.tempTransform.Find("ThirdStar").GetComponent<Image>().sprite = this.starOn;
            }
        }
